...
| NeoForge `Gui.leftHeight/rightHeight` HUD-stack accounting has no Fabric analog: `GuiLayerHooks.leftHeight()/rightHeight()` return the survival baseline constant (59 = 39 base + health + armor rows) and writes are dropped. Mekanism's own overlays floor at 59 anyway; drift only in creative (bar sits slightly high) or when another row (air/mount) is visible — cosmetic. MekaSuit bar's +8 bump is not propagated to other HUD layers | GuiLayerHooks (client/gui shim) | Phase 6 QA (or a HudRenderCallback-based per-frame accounting if overlap bites) |
| `IConfigScreenFactory` extension point is stored on the shim ModContainer but nothing reads it yet (NeoForge's mod-list screen doesn't exist on Fabric); config screen unreachable in-game until the Phase 5 ModMenu bridge consumes it via `getCustomExtension`. FCAP's `ConfigurationScreen` itself works (call-site adapted: mod id instead of container) | ModContainer + IConfigScreenFactory shims | Phase 5 (ModMenu integration) |
...
| `RegisterGuiLayersEvent` above/below-a-named-vanilla-layer positioning degrades to registration order: Fabric's HudRenderCallback has no vanilla-relative insertion slot, so `VanillaGuiLayers.*` ids are opaque ordering keys only. Mekanism's four HUD overlays (radiation, energy level, status, HUD) render in registration order relative to each other; their exact interleaving with vanilla layers is approximate | GuiLayerHooks + RegisterGuiLayersEvent shim (dispatch wired step 7) | Phase 4 step 7 (HudRenderCallback) / QA if overlay ordering looks wrong |
| OBJ models + robit render as the missing model: `ObjModel`/`ObjLoader`/`ElementsModel` are stubbed (their `bake()` returns the missing model) — Porting Lib was declined (client-models.md §7-A) and transmitters + robit are outside the Phase 4 milestone. Transmitters (universal cables, pipes, tubes, conductors) and the robit entity render as the missing/purple model; machine models (JSON + energy_cube/data_based loaders) are unaffected | ObjModel/ObjLoader/ElementsModel shims | later phase (real OBJ loader + faithful ElementsModel element baking) |
